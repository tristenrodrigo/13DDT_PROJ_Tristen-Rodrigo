...
class MainPage:
    ...
    def setup_ui(self):
        ...
        def open_sign_in(event):
            SignInPage(self.shared).sign_in_page()
            mainpage.withdraw()
        ...
        def open_manage_account(event):
            ManageAccountPage(self.shared).manage_account_page()
            mainpage.withdraw()
        ...
        def open_new_listing(event):
            ListingsPage(self.shared).new_listing_page()
            mainpage.withdraw()
        ...
        def search_and_open_listing(event):
            ...
            if len(results) == 1:
                ListingsPage(self.shared).listing_page(results[0][0])
            else:
